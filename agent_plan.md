...
| PC-09 | Repair the blank first step of client booking | Done locally; deploy pending | Live browser and API evidence showed the production catalog is empty (`GET /v1/services` returns `items: []`), so Step 1 rendered a heading over an empty list. It now presents the standard animated empty state, explicitly names the missing service details/duration/price, and offers an enquiry action. Focused tests, lint, production build and desktop browser verification pass. The supplied source names Nurse coaching and Holistic coaching but contains no approved duration or price, so no billable production offering was invented. |
