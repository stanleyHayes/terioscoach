...
| FND-05 | MongoDB Atlas cluster, collections & index design, seed tooling | DevOps + Backend | FND-04 | Done — the production API is connected to the remote database, readiness and authenticated account reads return 200, indexes are enforced at boot, and the confirmation-gated production seed provisioned the requested accounts and CMS content idempotently |
| FND-06 | Resend: domain verification, transactional template set (confirm, remind, reschedule, enquiry, feedback) | DevOps | FND-02 | **Verified by owner in Resend on 2026-08-30.** Transactional templates and delivery paths are implemented. Preserve the existing Resend DNS records during the Wix → Vercel web cutover. |
...
| FND-09 | Vercel projects `terios-web` + `terios-portal` + `terios-admin`: env wiring, preview deployments, API base URL config | DevOps | FND-01 | Done — all three projects exist with the correct `apps/web`, `apps/portal`, and `apps/admin` roots; required public environment variables are present; current production aliases return 200; `terioscoach.com`, `www`, `practice`, and `app` are attached to their intended projects. Wix DNS cutover remains tracked under LCH-08. |
...
| LCH-05 | Content placement: supplied copy, testimonials, service descriptions, portraits, blog images and logo masters refined and loaded through CMS-ready paths | Design | ADM-07 | Done and production-verified — the supplied archive was fully inventoried; 28 optimized portraits, 20 blog images and six final SVG logo variants are retained. The content-only production seed completed successfully on Render; live CMS pages, three posts, one consent-approved testimonial and public image delivery were verified. See `design/brand-assets.md`. |
...
| PC-09 | Repair the blank first step of client booking | Done and deployed | Live browser and API evidence showed the production catalog is empty (`GET /v1/services` returns `items: []`), so Step 1 rendered a heading over an empty list. It now presents the standard animated empty state, explicitly names the missing service details/duration/price, and offers an enquiry action. Focused tests, lint, production build and deployed browser verification pass. The supplied source names Nurse coaching and Holistic coaching but contains no approved duration or price, so no billable production offering was invented. |
...
| PROD-09 | Integrate the supplied brand archive | Done | 28 optimized portraits, 20 blog images, six final vectors and two legal templates retained; supplied logo mark now appears across public, portal, and dashboard shells; the authentic outdoor Theresa portrait replaces the generated booking fallback; journal cards render CMS cover imagery; all 48 editorial images remain available in the CMS picker; focused tests, scoped lint, API seed test, and public production build pass; disposition documented in `design/brand-assets.md` |
...
| PROD-12 | Repair responsive overlays, picker clipping, report spacing, and payment failure | Done | Explicit sidebar/backdrop/topbar/menu stacking and width guards verified at 390px; Availability cards permit picker overflow and the calendar flips above the field on wider screens; all-zero report series render a compact state instead of an empty chart; the production payment request now returns the intended structured 503 instead of panicking with a 500. Payment processing remains unavailable until a selected gateway's production credentials are configured. |
...
| PROD-14 | Replace native choice controls with an accessible branded system across every frontend | Done | Shared Base UI-backed select and checkbox primitives now power admin/portal preferences, form-builder types, and both consultation-room device menus; custom calendar/time controls received matching focus, elevation and overflow treatment; AST-aware lint guards prohibit native select/date/time/radio/checkbox regressions in web, admin and portal; 846 tests, workspace lint and all three production builds pass. |
| PROD-15 | Warm the marketing site with supplied photography | Done | Homepage service previews and approach, About story, Contact sidebar, FAQ lead-in, and every live service now use responsive supplied imagery; semantic service-theme matching plus stable custom-service fallbacks prevent text-only catalog rows; focused tests, scoped lint, desktop visual review, and the web production build pass |
...
| PROD-05 | Live edge/TLS/email/Lighthouse verification | Partially verified live | All four custom domains return 200 with valid auto-renewing TLS/HSTS/security headers; admin and portal are `noindex`; the public canonical, robots metadata and sitemap origin use `https://terioscoach.com`; Lighthouse reached Accessibility 100, Best Practices 100 and SEO 100 after contrast/image fixes. Performance remains variable under live cold-start conditions. Resend stayed verified after the DNS cutover. |
...
|---|---|
| FND-05, LCH-04 | Need the remaining production infrastructure credentials and operational verification |
| FND-10, LCH-03 live gate | Needs `SONAR_TOKEN` |
| LCH-05 | The client's own copy, photographs and testimonials |
| PROD-05, LCH-06 (performance baseline, video stress) | Custom domains and real two-network consultation traffic are still needed for final measurement |
...
| BOOK-02 | Add a safe, idempotent production catalogue seed | Done and live | Render job `job-daa9v8hf2nfc739m1kb0` succeeded; live `GET /v1/services` returns the active obligation-free 30-minute introduction and the 31 Aug–4 Sep Accra query returns 160 slots. Existing service and availability edits are preserved and unprovided paid prices are not invented |
...
| SEARCH-01 | Make practitioner global search functional | Done locally | The topbar control and Command/Ctrl-K now open an accessible search dialog, load client records on demand, search name/email/phone/tags, and navigate directly to the selected client record; 13 focused tests pass |
...
## 32. Dedicated service editor (13 Sep 2026)
...
## 34. Both-participant recordings and file downloads (15 Sep 2026)

Owner: Codex. Done. Investigation confirmed that MediaRecorder received only the remote video track, despite mixing both microphones. Both app recording paths now compose the local and remote cameras into one 1280x720 track with labeled, aspect-preserving participant tiles and camera-unavailable states. Track replacement is followed during recording; stopping releases only compositor resources, preserving call tracks. Existing recording consent and private storage authorization remain in place.
...
| Calendar 1 | Admin booked-session timezone selection | Done — admin calendar now has a themed display-timezone selector; weekly bookings, upcoming list rows, detail modal times, fetch bounds, and reschedule conversion all use the selected timezone. |
| Calendar 2 | Practitioner availability timezone and automatic conversions | Done — availability rules now carry the practitioner's authored timezone; admin availability has a themed working-timezone selector and saves time off in that timezone; public slot generation and booking/reschedule validation evaluate rules in the authored timezone while clients/admin can view converted times in their selected timezone. |
| Calendar 3 | Verify portal slots across timezones | Done — portal day keys now step by client-visible civil dates instead of fixed 24-hour UTC increments, preventing DST duplicate/skipped-day gaps; slot rendering and backend slot generation are covered across New York, Los Angeles, London, and Lagos for Accra-authored availability, with booking validation confirmed for a client-selected timezone. |
