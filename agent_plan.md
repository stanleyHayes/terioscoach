...
| LCH-08 | Domain cutover (Google-registered domain → Vercel/Render), SSL, go-live | DevOps | LCH-01–06 | In progress — Vercel has `terioscoach.com`, `www`, `practice`, and `app` attached to the correct projects, with CDN active. DNS is still hosted by Wix; the Wix login tab is open for owner authentication. Change only web/app records and preserve verified Resend MX/TXT/DKIM records. See `design/go-live-runbook.md` §7. |
| LCH-09 | Post-launch monitoring, uptime alerts, settling-in support period | DevOps + QA | LCH-08 | Active — `.github/workflows/production-smoke.yml` checks API readiness, the public site, both private app logins, security headers and `noindex` every 15 minutes. Its first manual production run (`33324521329`) passed all checks. `GET /v1/admin/ops/health` separately reports notification backlog, retry-exhausted failures, lockout spikes and payment-verification failures. External paging/notification routing remains an operator choice. |
...
| PROD-05 | Live edge/TLS/email/Lighthouse verification | Partially verified live | Vercel and Render aliases return 200 with valid TLS/HSTS/security headers; admin and portal are `noindex`; Lighthouse reached Accessibility 100, Best Practices 100 and SEO 100 after contrast/image fixes. Performance remains variable under live cold-start conditions and must be re-baselined after custom-domain cutover. Resend domain verification was confirmed by the owner. |
...
| LCH-08 | Wix DNS access requires owner authentication; the login tab is open. Preserve the verified Resend records during cutover. |
