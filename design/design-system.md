...
- **Clinical controls**: clients remain in a branded waiting state until the practitioner admits them; the API refuses WebRTC negotiation before admission. Starting a local recording opens a consent Modal for the other participant and cannot start on refusal. Leave opens a confirmation with **Leave only** for either side and **End for everyone** for practitioners. Settings use branded Selects for microphone, camera and supported speaker outputs.
