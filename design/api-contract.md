...
- **Provider selection:** `PAYMENT_PROVIDER=paystack` (default) or `stripe` picks the gateway at boot; the API surface is identical either way. Under Stripe, `initialize` creates a Checkout Session (success/cancel URLs derive from `PORTAL_URL`), the session ID is stored as the gateway reference, and `checkout.session.completed` on `/v1/webhooks/stripe` confirms the charge. Stripe webhook deliveries must carry a valid `Stripe-Signature` header — HMAC-SHA256 of `t.payload` under `STRIPE_WEBHOOK_SECRET` with a 5-minute timestamp tolerance, compared in constant time. Confirmation and refund go through `GET /v1/checkout/sessions/{id}` (refunds attach to the session's payment intent).
...
- **Webhook verification (Paystack):** requests to `/v1/webhooks/paystack` must carry a valid `x-paystack-signature` header — hex HMAC-SHA512 of the raw request body under `PAYSTACK_SECRET_KEY`, compared in constant time. Anything else answers 401 `invalid_signature`. The raw body is hashed before any JSON parsing.
- **Webhook handling:** only `charge.success` (Paystack) / `checkout.session.completed` (Stripe) mutates state, and only after a server-side verify against the gateway confirms the charge with amount and currency matching the stored payment. All other events, unknown references, already-`success` payments, and failed verify checks answer 200 without changes — **repeat deliveries are safe (idempotent on the gateway reference)**. A transient gateway failure during verify answers 502 so the provider retries.
...
- When the database **or** the selected gateway is not configured (its keys unset — for Stripe, either `STRIPE_SECRET_KEY` or `STRIPE_WEBHOOK_SECRET`), all `/v1/payments*` **and** `/v1/webhooks/*` routes answer 503 `service_unavailable`.
