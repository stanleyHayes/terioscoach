...
- **Webhook handling:** only `checkout.session.completed` mutates state, and only after a server-side Stripe verification confirms the charge with amount and currency matching the stored payment. Other events and duplicate deliveries are safely acknowledged.
