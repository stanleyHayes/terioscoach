...
- **Slot computation** (server-side, deterministic):
  - `from`/`to` are inclusive calendar dates interpreted in `tz`; range max 62 days. `tz` is an IANA name (e.g. `Africa/Accra`, `America/New_York`); defaults to `America/New_York` (US Eastern, the practice default) when omitted.
...
- `tz` (IANA name) tells the server which wall clock to evaluate the schedule in, exactly as on `/v1/availability/slots`; it defaults to `America/New_York` (US Eastern, the practice default) when omitted. It never changes the UTC timestamps stored or returned.
...
## Appointment readiness and account timezone — September 2026 update

This section supersedes the earlier unsigned-SOW and pre-checkout agreement gate for new booking executions. Checkout is permitted before document completion. Session ticket issuance **and redemption** require the current participant declaration, required client/guardian executions, coaching countersignatures where applicable, and completion of assigned booking-specific forms.

- `PATCH /v1/auth/me/timezone`, authenticated, accepts `{ "timezone": "America/New_York" }` and returns `{user}`. Accepts loadable regional IANA names or `UTC`; rejects abbreviations, local/default zones, offsets and whitespace. Auth user responses include optional `timezone`. Missing historical preferences remain missing until explicitly selected. No appointment instant changes when this preference changes.
- `POST /v1/bookings` additionally requires `participant: {name, under18, accurate, guardianName?, guardianRelationship?, guardianEmail?}`. `accurate` must be true. Minor declarations require guardian name, relationship and a valid contact email. Server assigns declaration actor/time/revision and appointment instant; request-supplied audit fields are overwritten. Guardian contact is declared, not externally verified. `bookingTimezone` preserves the requested display zone as booking audit context. Existing UTC timestamp contract is unchanged.
- `PATCH /v1/bookings/{id}/participant` accepts that participant object for the client owner or assigned practitioner, while pending/confirmed. It increments the participant revision and requires fresh executions; prior records remain available. A reschedule requires confirmation of the declaration for the new appointment instant. Legacy missing participants remain unknown.
- Booking lists include `serverTime` for client clock synchronization. Pending payment requests expire server-side at `startAt` into `cancelled` with `paymentExpired: true`; pending responses include `paymentExpiresAt`. A late verified payment is refunded rather than reviving the appointment. Optimistic booking revisions prevent a stale update overwriting a concurrent transition.
- `GET /v1/services/{id}/guardian-consent` returns `{agreement}` to authenticated users. Practitioner `GET /v1/guardian-consent` and `PATCH /v1/guardian-consent {body}` read/update versioned wording. First read initializes the owner-approved recommended default idempotently by practitioner/key. It is stored in the existing agreements collection; admin Settings provides the editor. Previous wording remains frozen in signatures.
- `GET /v1/bookings/{id}/agreements` returns `{items, ready, readinessMessage}` to the booking client or assigned practitioner only. Each item contains `required`, `signed`, `agreement`, optional `signature`, service `fee`, and applicable `consentVersion`. `signed` describes the client/guardian execution; `ready` also considers countersignatures, participant validity and assigned forms. Minor bookings always require the guardian consent document, even if the service has no other required agreements.
- New `POST /v1/agreements/{id}/sign` and `/submit` executions include `{bookingId, agreementVersion, signerRole: "client"|"guardian", signedName, acknowledged: true, consentVersion?}`. `/submit` adds `statementOfWork`. Guardian `consentVersion` must equal the wording presented. Validation precedes idempotency lookup. Execution identity binds booking, participant revision, consent version (for minors), agreement version and signer role. A different child, rescheduled declaration or updated wording cannot inherit an old execution.
- Holistic SOW answers: `{clientName, effectiveDate: "YYYY-MM-DD", initialTermMonths, monthlyFee}`. Nurse SOW answers: `{clientName, effectiveDate, package, monthlyFee}` (legacy `initialTermMonths` remains readable). These reflect the supplied source PDF blanks. New nurse executions require a nonblank package. The participant name and fee must match the booking/service; `monthlyFee` is a read-only catalog value such as `USD 250.00`. Answers and typed signature persist together. Dates remain civil dates, never converted through a timezone.
- Legacy unsigned `/submit` calls without a booking/signature remain supported and are labelled **Submitted — unsigned (legacy)**. They cannot satisfy new readiness. Legacy account signatures remain in `agreement_signatures`; new contextual evidence lives in additive `agreement_executions`. Existing uniqueness/evidence is not deleted or rewritten. Evidence digest uses millisecond timestamps to match Mongo persistence precision.
- Signature responses expose `signerRole`, `participantName`, `contextId`, `consentVersion`, `statementOfWork`, `submittedAt`, and `archiveStatus`. The authoritative signed snapshot is immediately available for PDF generation. `archiveStatus=pending` means the convenience archive still needs delivery. Immutable execution keys deduplicate original and countersigned PDF archives independently. PDFs show original answers, exact wording, guardian evidence and applicable practitioner signatures.

Required-document edits and changes to a service's assigned documents durably request signatures for relevant future bookings. Email delivery and PDF archival do not determine whether the original valid signature write succeeds. MongoDB Atlas/replica-set transactions are required by the mutation journal. See the implementation document and `design/in-app-notifications.md` for rollout, recovery and owner-run acceptance checks.
