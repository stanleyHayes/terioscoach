...
export function FAQSearch({ faqs }: FAQSearchProps) {
  ...
  const groups = useMemo(() => groupFAQs(searchFAQs(faqs, query)), [faqs, query]);
  const matchCount = useMemo(() => searchFAQs(faqs, query).length, [faqs, query]);
  ...
  return (
    <div className="flex flex-col gap-14">
      <div className="rounded-[2rem] bg-eucalyptus-50 p-6 sm:p-8">
        ...
        <label htmlFor="faq-search" className="block text-sm font-medium text-ink">
        ...
        <p aria-live="polite" className="mt-2 min-h-[1.25rem] text-[13px] text-ink-faint">
          {searching
      ...
      {matchCount === 0 ? (
        <div className="mx-auto flex max-w-[360px] flex-col items-center px-6 py-12 text-center">
          ...
          <p className="mt-2 text-sm leading-[1.55] text-ink-muted">
            Try a different word — or send the question over and it will be
            answered directly.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-12">
          {groups.map(([category, entries]) => (
            <section key={category} aria-labelledby={`faq-group-${slugify(category)}`} className="grid gap-5 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-12">
              <h2
