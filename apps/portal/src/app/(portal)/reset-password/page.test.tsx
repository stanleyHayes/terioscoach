...
const searchParams = vi.hoisted(() => ({ current: new URLSearchParams("token=reset-token") }));
...
describe("client password reset", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    searchParams.current = new URLSearchParams("token=reset-token");
  ...
  it("sends the token from the link with the new password", async () => {
    ...
    await waitFor(() =>
      expect(resetPassword).toHaveBeenCalledWith("reset-token", "a sufficiently long password"),
    );
    ...
    // tells someone whose account was taken that they have it back.
    expect(screen.getByText(/previous sessions have been signed out/i)).toBeTruthy();
  });
  ...
  it("refuses a mismatched confirmation", async () => {
    ...
    // Locking someone out of their own care record with a typo they cannot
    // see is the failure this prevents.
  ...
  it("distinguishes an expired link from a server problem", async () => {
    ...
    // moment. Telling someone the wrong one wastes their evening.
  ...
  it("falls back to a retry message for any other failure", async () => {
    ...
    expect((await screen.findByRole("alert")).textContent).toMatch(/couldn't reset your password/i);
  ...
  it("sends the client to sign in once the password is changed", async () => {
    render(<ResetPasswordPage />);
    fill("a sufficiently long password");

    expect(await screen.findByRole("link", { name: /sign in with new password/i })).toBeTruthy();
