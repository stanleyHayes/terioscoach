...
/**
 * The video room's whole lifecycle in one hook (CX-05/CX-06).
 *
...
export function useVideoRoom(bookingId: string): VideoRoom {
  const { session, onTokensRefreshed, touchActivity } = useAuth();
  ...
  const join = useCallback(
    (options?: JoinOptions) => {
      ...
      void (async () => {
        ...
        try {
          granted = await videoApi.join(session, { onTokensRefreshed }, bookingId);
        } catch (failure) {
          setError(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [session, onTokensRefreshed, bookingId, refreshDevices],
  );
