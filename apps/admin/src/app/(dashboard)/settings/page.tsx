...
export default function SettingsPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <Card><div className="mb-5 flex items-center gap-3"><BookOpen className="text-primary"/><div><h2 className="font-display text-xl font-semibold">Preferences & tutorial</h2><p className="text-sm text-ink-muted">Control reminders and restart the guided workspace tour.</p></div></div><div className="grid gap-5 sm:grid-cols-2"><label className="text-sm font-medium">Timezone<select value={timezone} onChange={(e)=>setTimezone(e.target.value)} className="mt-2 h-11 w-full rounded-xl border border-border-strong bg-surface-raised px-3"><option value="Africa/Accra">Accra (GMT)</option><option value="Europe/London">London</option><option value="America/New_York">New York</option></select></label><label className="flex items-center gap-3 rounded-xl border border-border p-4 text-sm"><input type="checkbox" checked={reminders} onChange={(e)=>setReminders(e.target.checked)} className="size-5 accent-primary"/>Email session reminders</label></div><div className="mt-5 flex flex-wrap gap-3"><Button type="button" onClick={savePreferences}>Save preferences</Button><Button type="button" variant="secondary" onClick={()=>{ localStorage.removeItem("terios.admin.onboarding.complete"); window.dispatchEvent(new Event("terios:onboarding")); }}>Restart tutorial</Button></div></Card>
  </div>;
}
