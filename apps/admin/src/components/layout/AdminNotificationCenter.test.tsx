...
vi.mock("@/lib/use-resource", () => ({ useResource: resource, useAction: action }));
...
const item = { id: "n1", title: "A form is ready", link: "/clients", kind: "form_assigned", read: false, createdAt: "2026-09-17T12:00:00Z" };
beforeEach(() => {
 vi.clearAllMocks();
...
it("shows persistent unread counts and the correct destination", () => {
 render(<AdminNotificationCenter />); open();
 expect(screen.getByText("9+")).toBeTruthy();
 expect(screen.getByRole("dialog", { name: "Practice notifications" })).toBeTruthy();
 expect(screen.getByRole("link", { name: /A form is ready/ }).getAttribute("href")).toBe("/clients");
});
...
it("shows loading, errors and an honest empty state", () => {
 ...
 const view = render(<AdminNotificationCenter />); open();
 expect(screen.getByLabelText("Loading notifications")).toBeTruthy();
 resource.mockReturnValue({ data: null, error: "offline", refresh }); view.rerender(<AdminNotificationCenter />);
 expect(screen.getByRole("alert")).toBeTruthy();
 resource.mockReturnValue({ data: { items: [], unreadCount: 0 }, error: null, refresh }); view.rerender(<AdminNotificationCenter />);
...
it("closes on Escape and returns focus to the bell", () => {
 render(<AdminNotificationCenter />); open(); fireEvent.keyDown(document, { key: "Escape" });
...
it("refuses external, protocol-relative and backslash links", () => {
 ...
 expect(notificationHref("/\\example.com")).toBeNull();
 expect(notificationHref("/clients")).toBe("/clients");
});
