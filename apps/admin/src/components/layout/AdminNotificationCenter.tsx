...
export function AdminNotificationCenter() {
  ...
  const resource = useResource(notificationFeedApi.list, []);
  ...
  return <div className="sm:relative" ref={rootRef}>
    ...
    {open ? <section role="dialog" aria-label="Practice notifications" className="terios-popover absolute inset-x-3 top-full z-[55] mt-2 w-auto sm:inset-x-auto sm:right-0 sm:top-auto sm:w-96 overflow-hidden rounded-3xl border border-border bg-surface-raised text-ink shadow-2xl">
      <header className="flex items-start justify-between gap-2 border-b border-border px-5 py-4">
        <div><p className="text-sm font-semibold">Practice notifications</p><p className="mt-0.5 text-xs text-ink-muted">{count ? `${count} unread updates` : "Your recent updates"}</p></div>
        <div className="flex">
